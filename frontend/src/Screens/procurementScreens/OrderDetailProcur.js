...
const OrderDetailsProcur = () => {
  ...
  // Optimistic deliver handler
  const deliverHandler = async () => {
    try {
      const updatedOrder = await deliverOrderProcurement(orderId).unwrap();

      // Update frontend immediately
      setOrder((prev) => ({
        ...prev,
        isDelivered: true,
        deliveredAt: updatedOrder.deliveredAt || new Date().toISOString(),
      }));

      toast.success("Order Delivered and Stock Incremented!");

      setTimeout(() => navigate("/warehouse"), 1000);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-200 p-6 space-y-8">
      <div
        className="bg-gray-800 rounded-lg shadow-lg p-6 grid md:grid-cols-2 gap-6 relative"
        style={{ boxShadow: "0 0 20px rgba(59, 130, 246, 0.5)" }}
      >
        <div className="space-y-2">
          <h2 className="text-2xl font-semibold mb-4">Order Info</h2>
            ...
            {order?.orderItems?.map((item) => (
              ...
              >
                ...
                <td className="py-3 px-4">
                  {item.qty}{" "}
                  {order.isDelivered && (
                    <span className="text-green-400 ml-1">+{item.qty}</span>
                  )}
                </td>
                <td className="py-3 px-4">{item.price || "0"}</td>
                <td className="py-3 px-4">{item.supplier || "N/A"}</td>
      ...
      {!order?.isDelivered && (
        <div className="flex justify-end">
          <button
            onClick={deliverHandler}
            disabled={deliverLoading || order.isDelivered}
            className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-2xl font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {deliverLoading ? <Loader /> : "Deliver Order to Warehouse"}
          </button>
        </div>
...
const OrderDetailsProcur = () => {
  ...
  const [deliverOrder, { isLoading: deliverLoading }] =
    useDeliverOrderMutation();
  const [receiveOrder, { isLoading: receiveLoading }] =
    useReceiveOrderMutation();
  ...
  // Status badge helper
  const statusBadge = (isDelivered, isReceived) => {
    if (isReceived) {
      return (
        <span className="bg-green-600 text-white px-2 py-1 rounded-full text-xs">
          Received
  ...
  // Deliver handler (procurement = stock IN)
  const deliverHandler = async () => {
    try {
      ...
      toast.success("Order delivered (Procurement → stock ready to receive)");
    } catch (err) {
      toast.error(err?.data?.message || err.error || "Failed to deliver order");
    }
  };

  // Receive handler (procurement = finalize stock IN)
  const receiveHandler = async () => {
    try {
      const updatedOrder = await receiveOrder(orderId).unwrap();

      setOrder((prev) => ({
        ...prev,
        isDelivered: true,
        isReceived: true,
        receivedAt: updatedOrder.receivedAt || new Date().toISOString(),
      }));

      toast.success("Order received and stock incremented!");
    } catch (err) {
      toast.error(err?.data?.message || err.error || "Failed to receive order");
    }
  };
  ...
  // Restrict: only PURCHASE REQUISITION should use this page
  const isPurchaseReq = order?.requisitionSteps === "PURCHASE REQUISITION";
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-200 p-6 space-y-8">
          ...
          <tbody>
            {order?.orderItems?.map((item) => (
              ...
              >
                ...
                <td className="py-3 px-4">{item.supplier || "N/A"}</td>
                <td className="py-3 px-4">
                  {statusBadge(order?.isDelivered, order?.isReceived)}
                </td>
                {!order?.isDelivered && (
                  <td className="py-3 px-4">
                    <button className="bg-indigo-600 hover:bg-indigo-700 px-3 py-1 rounded-lg text-white">
      ...
      {/* Action Buttons (only show for Purchase Requisitions) */}
      {isPurchaseReq && (
        <div className="flex flex-wrap gap-4 justify-end">
          ...
          {order?.isDelivered && !order?.isReceived && (
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-2xl font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {receiveLoading ? <Loader /> : "Receive Order"}
            </button>
          )}
        </div>
      )}
