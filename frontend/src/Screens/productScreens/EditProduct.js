...
import { useParams, useNavigate } from "react-router-dom";
import { toast } from "react-toastify";
import { getProduct, updateProduct, getProducts } from "../../redux/productSlice.js";
...
const EditProduct = () => {
  ...
  const { product, isLoading, isError, message } = useSelector((state) => state.products);
  const { Uom } = useSelector((state) => state.category);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center p-4">
      <div
        className="w-full max-w-xl p-6 rounded-3xl bg-gray-800 text-gray-200
        ...
        style={{
          borderImageSlice: 1,
          borderWidth: "3px",
          borderImageSource: "linear-gradient(to right, orange, blue, purple)",
        }}
      >
        <h1 className="text-2xl md:text-3xl font-bold text-center mb-6 text-white">Edit Product</h1>
        {isError && <p className="text-red-400 mb-4">{message}</p>}
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                value={formData.name}
                onChange={handleInputChange}
                className="border border-gray-600 p-2 md:p-3 w-full rounded-lg bg-gray-700 text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
          ...
          <div className="md:col-span-2 flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-4 mt-4">
            <button
              type="submit"
              className={`flex-1 py-2 md:py-3 bg-indigo-600 text-white rounded-xl font-semibold hover:bg-indigo-800 transition ${isLoading ? "opacity-70 cursor-not-allowed" : ""}`}
              disabled={isLoading}
            >
              {isLoading ? "Updating..." : "Update Product"}
            </button>
            ...
            <button
              type="button"
              onClick={() => navigate("/dashboard")}
              className="flex-1 py-2 md:py-3 bg-red-700 text-white rounded-xl font-semibold hover:bg-red-900  transition"
            >
              Cancel
            </button>
          </div>
