...
const PlaceOrder = () => {
  ...
  // --- DEBUG: inspect actual Redux objects ---
  useEffect(() => {
    console.log("requisitionSteps full object:", requisitionSteps);
    console.log("approvedData full object:", approvedData);
  }, [requisitionSteps, approvedData]);

  // --- DYNAMIC REQUISITION TYPE ---
  // Tries multiple known paths before defaulting to "Not set"
  const requisitionType =
    requisitionSteps?.type ||
    requisitionSteps?.method ||
    requisitionSteps?.data?.type ||
    approvedData?.requisitionType ||
    approvedData?.type ||
    approvedData?.data?.type ||
    "Not set";
  ...
  const placeOrderHandler = async () => {
    try {
      ...
      console.error("Failed to place order:", err);
      toast.error(err?.data?.message || "Failed to save requisition.");
    }
  };
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 py-8 px-4 md:px-10">
      ...
      {cartItems.length === 0 ? (
        ...
        <>
          ...
          <div className="sm:hidden flex flex-col gap-4">
            {cartItems.map((item) => (
              <div className="bg-gray-800 p-4 rounded-2xl shadow-[0_0_12px_rgba(59,130,246,0.5)] border border-gray-700 flex flex-col space-y-2">
                <div className="flex justify-between items-center">
                  <div>
                    <p className="font-semibold text-lg text-gray-100">{item.name}</p>
                    <p className="text-blue-400 font-semibold">{item.category || "N/A"}</p>
