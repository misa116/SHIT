...
const adjustStockByRequisitionType = async (order) => {
  const requisitionType =
    order?.approvedData?.requisitionType ||
    order?.approvedData?.type ||
  ...
  for (let item of order.orderItems) {
    ...
    if (requisitionType.toUpperCase() === "PURCHASE REQUISITION") {
      product.stock += item.qty; // ✅ add stock
    } else if (requisitionType.toUpperCase() === "FACTORY REQUISITION") {
